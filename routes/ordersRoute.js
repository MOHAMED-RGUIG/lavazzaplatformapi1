...
router.post('/placeorder', async (req, res) => {
    let pool;
    try {
        const { subtotal, currentUser, cartItems, codeClient, modalitePai, dateCmd } = req.body;
        ...
        for (let item of cartItems) {
            ...
            const query = `
                ...
                (SOHNUM, ORDDAT, BPCORD, ITMREF, ITMDES, QTY, GRAT, NETPRI, TOTLIN, MODPAY, [USER], CREDAT) 
                VALUES (@SOHNUM, @dateCmd, @codeClient, @itmref, @itmdes, @qty, @grat, @netpri, @totlin, @modalitePai, @userID, GETDATE())
            `;
